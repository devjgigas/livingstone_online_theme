 ...
 */
function livingstone_theme_status_messages($variables) {
  ...
  foreach ($message_list as $type => $messages) {
    ...
    if (count($messages) > 1) {
      ...
      foreach ($messages as $message) {
        $output .= '  <li>' . (!module_exists('devel') ? filter_xss_admin($message) : $message) . "</li>\n";
      }
      $output .= " </ul>\n";
    }
    else {
      $output .= (!module_exists('devel') ? filter_xss_admin($messages[0]) : $messages[0]);
    }
